...
interface NavItem {
  key: string; // Key will be the string sourced from common.json
  href: string;
  name: string; // Add name from common.json
  description?: string; // Add description from common.json
}

interface NavCategory {
  label: string; // e.g., 'acquire', 'build', 'convert'
  items: NavItem[];
}
...
interface NavLink {
  label: string; // e.g., 'product', 'resources', 'pricing'
  href?: string;
  isMegaMenu?: boolean;
  ...
  const transformData = (): NavLink[] => {
    ...
    for (const [key, product] of Object.entries(commonData.products)) {
      ...
      productsByCategory[product.category].push({
        key: key, // Key is directly the string from common.json
        href: product.href,
    ...
    const resourceItems: NavItem[] = commonData.links
      .filter(link => link.href.startsWith('/resources/'))
      .map((link, index) => ({
          // Use href or index as key, name for display
    ...
    const pricingLink = commonData.links.find(link => link.href === '/pricing');
    const exploreOddleLink = commonData.links.find(link => link.href === '/');
                  ...
                  <div key={category.label} className="ml-2 grid gap-1">
                    ...
                    {category.items.map(item => {
                      ...
                      return (
                        ...
                        >
                          {item.name} {/* Display name directly */}
                        </Link>
                      )
                    })}
                  </div>
                            ...
                            <ul className="space-y-1">
                              {category.items.map((item) => {
                                ...
                                return (
                                  <ListItem
                                    key={item.key}
                                    title={item.name} /* Display name directly */
                                    href={item.href}
                                  >
                                    {item.description} {/* Display description directly */}
                                  </ListItem>
                                );
                              })}
                            </ul>
                          ...
                          <NavigationMenuLink asChild>
                            ...
                            >
                              <div className="mb-2 mt-4 text-lg font-medium">
                                {link.promo.title} {/* Display promo title directly */}
                              </div>
                              <p className="text-sm leading-tight text-muted-foreground">
                                {link.promo.description} {/* Display promo description directly */}
                              </p>
                            </a>
                          </NavigationMenuLink>
                        </li>
